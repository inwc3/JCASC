package hiveworkshop.blizzard.casc.vfs;

import hiveworkshop.blizzard.casc.Key;
import hiveworkshop.blizzard.casc.StorageReference;
import hiveworkshop.blizzard.casc.nio.MalformedCASCStructureException;
import hiveworkshop.blizzard.casc.storage.BankStream;
import hiveworkshop.blizzard.casc.storage.Storage;
...
import static hiveworkshop.lang.Hex.compareUnsigned;
...
public final class VirtualFileSystem {
	...
	public final class PathResult {
		 ...
		 */
		private PathResult(PathNode node, byte[][] pathFragments) {
			this.node = node;
		...
		public boolean existsInStorage() {
			...
			if (isFile()) {
				...
				for (int fileReferenceIndex = 0; fileReferenceIndex < fileReferenceCount; fileReferenceIndex += 1) {
					final hiveworkshop.blizzard.casc.vfs.StorageReference fileReference = fileNode.getFileReference(fileReferenceIndex);
		...
		public boolean isTVFS() {
			...
			final FileNode fileNode = (FileNode) node;
			final hiveworkshop.blizzard.casc.vfs.StorageReference fileReference = fileNode.getFileReference(0);
	...
	private static int compareNodePathFragments(final byte[][] pathFragments, final int fragmentIndex,
		...
		byte[] f1 = new byte[Math.min(fragmentOffset + nodeFragment.length, fragment.length)];
		byte[] f2 = new byte[nodeFragment.length];

		System.arraycopy(fragment, fragmentOffset, f1, 0, f1.length);
    System.arraycopy(nodeFragment, 0, f2, 0, f2.length);
	 ...
	 */
	private static boolean equalNodePathFragments(final byte[][] pathFragments, int fragmentIndex, int fragmentOffset,
			final PathNode node) {
		final int nodeFragmentCount = node.getPathFragmentCount();
		if (nodeFragmentCount == 0) {
			// nodes without fragments have no path fragment presence so always match
      ...
      byte[] f2 = new byte[nodeFragment.length];
      System.arraycopy(fragment, fragmentOffset, f1, 0, f1.length);
      System.arraycopy(nodeFragment, 0, f2, 0, nodeFragment.length);

			result = result && Arrays.equals(f1, f2);
			fragmentOffset = 0;
			nodeFragmentIndex += 1;
		}
	...
	public VirtualFileSystem(final Storage storage, final Map<String, String> buildConfiguration) throws IOException {
		...
		final StorageReference rootReference = new StorageReference(CONFIGURATION_KEY_PREFIX + ROOT_KEY, buildConfiguration);
	...
	private ByteBuffer fetchStoredBuffer(final StorageReference storageReference) throws IOException {
		...
		try {
			while (bankStream.hasNextBank()) {
        ByteBuffer bank = bankStream.getBank(null);
        storedBuffer.put(bank.array());
      }
