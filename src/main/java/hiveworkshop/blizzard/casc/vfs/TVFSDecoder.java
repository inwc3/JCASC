...
public class TVFSDecoder {
	...
	public List<PathNode> decodeContainer()
			throws MalformedCASCStructureException {
		return decodeContainer(pathBuffer);
	}

	private List<PathNode> decodeContainer(final ByteBuffer pathBuffer)
			throws MalformedCASCStructureException {
		...
		try {
			...
			for (int i = 0; i < referenceCount; i += 1) {
				...
				try {
					...
					final StorageReference reference = new StorageReference(offset, size, new Key(encodingKeyDecoder), physicalSize, actualSize);
	...
	public TVFSFile loadFile(final ByteBuffer fileBuffer) throws IOException {
		...
		if (localBuffer.remaining() < IDENTIFIER.remaining() || !localBuffer.limit(IDENTIFIER.remaining()).equals(IDENTIFIER)) {
		...
		final TVFSFile tvfsFile = new TVFSFile(version, flags, encodingKeySize, patchKeySize, maximumPathDepth, rootNodes);
