package hiveworkshop.blizzard.casc.info;

import hiveworkshop.blizzard.casc.nio.MalformedCASCStructureException;
import hiveworkshop.nio.ByteBufferInputStream;
...
public class Info {
	...
	public Info(final ByteBuffer fileBuffer) throws IOException {
		try (final ByteBufferInputStream fileStream = new ByteBufferInputStream(fileBuffer);
				final Scanner lineScanner = new Scanner(fileStream, "UTF8")) {
			final String[] encodedFieldDescriptors = separateFields(lineScanner.nextLine());
			for (String encodedFieldDescriptor : encodedFieldDescriptors) {
				fieldDescriptors.add(new FieldDescriptor(encodedFieldDescriptor));
			}

			while (lineScanner.hasNextLine()) {
				records.add(new ArrayList<>(asList(separateFields(lineScanner.nextLine()))));
			}
