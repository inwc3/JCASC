package hiveworkshop.blizzard.casc.storage;
...
public class Storage implements AutoCloseable {
	...
	public Storage(final Path dataFolder, final boolean useOld, final boolean useMemoryMapping) throws IOException {
		...
		try (final DirectoryStream<Path> indexFileIterator = Files.newDirectoryStream(folder, "*." + INDEX_FILE_EXTENSION)) {
		...
		final HashMap<Integer, ArrayList<IndexFileNameMeta>> metaMap = new HashMap<Integer, ArrayList<IndexFileNameMeta>>(INDEX_COUNT);
      ...
      ArrayList<IndexFileNameMeta> bucketList = metaMap.get(index);
			if (bucketList == null) {
